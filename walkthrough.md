...
- **Registration Form**: Click "Register Now" on an event card to open a gorgeous **Full-Screen Split-Pane Event Details & Registration Interface** (`width: 100vw; height: 100vh;`) watermarked with a beautiful, highly transparent background illustration of the user-provided Cartoon Baby Krishna (`cartoon_baby_krishna.jpg`). The left pane displays the organiser's custom banner image, title, category, formatted date, venue, and full guidelines (hiding the banner picture completely if none is selected by the organiser). The right pane displays the student registration form directly, allowing complete enrollment in one screen. The terms agreement checkbox has been removed and the close button is layered safely on top with `z-index: 999` to ensure flawless pointer-event click triggers.
